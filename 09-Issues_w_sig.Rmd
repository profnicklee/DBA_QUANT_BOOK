...
Now, that is just the *expected* level for every 100 tests. If you remember from the discussion in Chapter 6 on Confidence Intervals, it may not be you get exactly 5 false positives in 100 tests, just that you would expect the long run average number of false positives to be 5% in a world of null hypotheses.
...
Recalling the laws of probability then... *P*(at least one 6) = 1 - *P*(no 6) = 1 - .579 = .421

So, *now* we have a 42% probability of at least one 6 in 3 rolls, even though the probability of getting a 6 in a single roll is unchanged.

We can calculate the familywise error rate for any set of multiple comparisons in exactly the same way.

In the case of the Ed Sheeran study, we are doing 3 pairwise comparisons (i.e. 3 tests) across the 3 groups, with a stated significance level of 0.05.
...
Kind of scary right? There are many ways to deal with it, but they pretty much all amount to making some correction to the required significance level, making it more stringent to reduce the false positives.
...
We can use these to create a nifty chart with the error bars...drawing from the code used by Spiegelhalter for Figure 9.4 in his book, available on his github:
...
Where $\alpha_b$ = Bonferroni-adjusted critical p value 
...
But, as I suggested above, that might not always be the best idea. Indeed, if you want to totally avoid any chance of a false positive, why not simply make the required alpha 0? Then, you would never get a false positive. Of course, you would never get a **true positive** either. 
...
To calculate power, we need the parameters of the experiment and analysis design. As such, it is easily possible to do this before collecting data, and to design your studies around it. Really, we should do this a lot more in business and management - it's routine in fields like medicine.
