...
First, we find some data to play with. We'll begin by looking at some COVID data. Here is some data from Johns Hopkins University as at April 26 2022 by country about cases, deaths, and mortality for COVID. I'm going to use the top 20 countries in terms of raw number of cases
...
Above, we simply graphed a single variable of interest, and showed that some very basic changes to how it was displayed had the potential to 'lead' the reader to different conclusions.

Now, let's think about the idea of and even simpler way to describe date - by summarizing it with just some numbers.
...
What we have here are the classic 'summary statistics' for 2020 GDPph.
...
What we have here now includes the range (the bottom number) and the interquartile range (above that), which are both useful measures of spread.
...
Specifically, the **median** and **interquartile range** are less sensitive to outliers as measures of central tendency and range respectively than their counterparts the **mean** and the **range**. So, that's why you commonly see median in particular used so frequently in these sorts of situations.
...
We can see here that the data is very skewed, and there are some very significant outliers.
...
So, we can see here that most of the data is clustered towards the bottom of the range - i.e. very low GDPph. This is of course the same conclusion as the historgram allowed. But, the box plot also gives us plenty of other information about the variable.

But, while box plots are nice, they can also obscure important facts about the distribution. Adding the jitter helps, but there are other ways to display this type of data that make the distribution more obvious.
...
These are really cool in my view. And, if you are asking me, my preference would be to combine jittered box plots, which show the basics, and emphasize the outliers and summary numbers, with ridge plots, which show a really nice visualization of the distributions of each variable. 
